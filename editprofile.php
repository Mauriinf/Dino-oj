<?php
	...
	if(isset($_REQUEST["form"]) && ($_REQUEST["form"] == true)):
		...
		$query = "select * from users where user_id <> '{$_SESSION['UserName']}' AND Email = '$email'";
		$rs = mysqli_query($enlace,$query) or die('Algo anda mal: ' . mysqli_error());
		$validate = false;
		
         
		if(mysqli_num_rows($rs)==0){
			$query = "update users SET  Nombre= '{$nombre}',Apellidos= '{$apellido}', Password= '{$password}', Ubicacion= '{$ubicacion}', Institucion= '{$escuela}', Email= '{$email}' WHERE  user_id =  '{$_SESSION['UserName']}' LIMIT 1 ;"; 
			$rs = mysqli_query($enlace,$query) or die("Error al actualizar". mysqli_error($enlace));
			$validate = true; 
		}
		else{
			$sms="Ya existe un usuario con este mail intenta con otro."; 
...
<head>
	...
	<title>Dino Online Judge - Editar Perfil</title>
	<style>
	...
	<div class="post" style="background:white; " >
		...
		<?php if(isset($_REQUEST["form"]) && ($_REQUEST["form"] == true)): ?>
			<?php if(true): ?>
				<h2>Yeah !!   &nbsp;&nbsp;&nbsp;  :-)</h2>
				<p>
					Hola <b><?php echo $_SESSION['UserName'] ?></b> ! Haz editado correctamente tus datos !<br/> Saluda a Dino de mi parte. <br><div align="right">Atte: <b>El script de edicion de perfiles. </b></div>
				</p>
			<?php else: ?>
				<h2>Ups :-(</h2>
				<p>
					Ya existe un usuario con este <b>email</b>. Puedes intentar resetear tu contrase&ntilde;a o bien <a href="javascript:history.go(-1);">regresar</a> e intentar de nuevo. <br><div align="right">Atte: <b>El script de edicion de perfiles. </b></div>
				</p>
			<?php endif; ?>
		<?php else: ?>
			<p>
			...
			<input type="text" id="nick" name="nick" class="text" required="" value="<?php echo $datos['user_id']; ?>" readonly/>
			...
			<input type="password" id="rpassword" name="re_password" class="text" required=""/>
			<div id="checkrpassword" class=""></div>
			<div class="" id="divchearsisoniguales"></div>
			<label for="nombre">
				Nombre Completo:
			</label>
			<input type="text" id="nombre" name="nombre" class="text" required="" value="<?php echo $datos['Nombre']; ?>"/>
			<div id="checknombre" class=""></div>
			<label for="apellidos">
				Apellidos:
			</label>
			<input type="text" id="apellidos" name="apellidos" class="text" required="" value="<?php echo $datos['Apellidos']; ?>"/>
			<div id="checkapellidos" class=""></div>
			<label for="email">
				Correo :
			</label>
			<input type="text" id="email" name="email" class="text" required="" value="<?php echo $datos['Email']; ?>"/>
			...
			<input type="text" id="escuela" name="escuela" class="text" required="" value="<?php echo $datos['Institucion']; ?>"/>
		...
		<?php 
			if( ! isset($_SESSION['UserName'] ) ){
				?> <div align="center"><a href="href='login.php'">Debes iniciar sesion para poder editar tus datos.</a></div> <?php
					...
					$query = sprintf("select * FROM users WHERE user_id='%s'", mysqli_real_escape_string($enlace,$_SESSION['UserName']));
					//echo $query;
...
if(email==""){
  	...
  	if (!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,4})+$/.test(email))){
		$("#checkemail").html("<div class='alert alert-danger'> Ingrese un email valido.<input value='error' type='hidden' name='passwordchecker'></div>");
  		...
  		{	  			 
		$("#checkemail").html("<div class='alert alert-success'> Email correcto.<input value='error' type='hidden' name='passwordchecker'></div>");
  		}
  	}
}  
