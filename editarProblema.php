...
session_start();
	...
	if ( !empty($_POST))
	{
		...
		{			
			...
			$registro = EditarProblema($id,$titulo,$autor,$tiempo,$memoria,$descripcion,$entrada,$salida,$ejemploentrada,$ejemplosalida,$arch);			
			if($registro > 0)
				{				
...
<div class="wrapper">
	...
	<div class="post" style="background:white;">
		...
		<?php if(!isset($_REQUEST["form"])): ?>
			...
			<textarea id="entrada" row="10" cols="5"  required="" name="entrada" ><?php echo $datos['Entrada']; ?></textarea>
			<div id="checkentrada" class=""></div>
			<label for="salida">
				Salida
			</label>
			<textarea id="salida" row="10" cols="5" required="" name="salida" ><?php echo $datos['Salida']; ?></textarea>
